...
public class Polygon implements Serializable {
	...
	public boolean contains(Point pt) {
		...
		for (i = 0; i < n; i++) {
			if (((points.get(i).getY() >= pt.getY()) != (points.get((i + 1) % n).getY() >= pt.getY())) && (pt.getX() <= (points.get((i + 1) % n).getX() - points.get(i).getX())
					* (pt.getY() - points.get(i).getY()) / (points.get((i + 1) % n).getY() - points.get(i).getY()) + points.get(i).getX())) {
				c = !c;
			}
		}
		return c;
